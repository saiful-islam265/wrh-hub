<div class="col-xl-8">
	<div class="dashboard-page__order">
		...
		<div class="dashboard-page__order-body" id="wrh-quick-shipping-list">
			<table class="table table-order">
				...
				if ( $all_orders->have_posts() ) {
					while ( $all_orders->have_posts() ) {
						...
						$order_status = !empty($order_status)? $order_status : 'In Production';
						...
						if('quick_stock_request' == get_post_type()){
							...
							<tr style="background-color: #ff2f2fc2; color:#ffffff00">
								...
								<button role="button" class="btn btn-waiting quick-ship-wrh-ready_pick"<?php echo esc_attr( $backgroundg_color ); ?> data-variation_id="<?php echo the_title(); ?>" data-req_stock="<?php echo intval( $req_stock_quantity ); ?>" data-size_attr='<?php echo  $size_attr; ?>' class="quick-ship-wrh-ready_pick" data-nonce="<?php echo esc_attr( wp_create_nonce( 'quick_ship_wrh_nonce' ) ); ?>">
									<?php esc_html_e( 'Ready for pickup', 'hoodslyhub' ); ?>
								</button>
								</td>
								<td>HypeMill</td>
								<td data-title="Order Id">
									<p style="color:#fff">Stock Requested - <?php echo $req_stock_quantity; ?></p>
								</td>
								<td data-title="Order Id">
									<p style="color:#fff"><?php echo $size_attr; ?></p>
								</td>
								<td></td>
								<td></td>
							</tr>
						...
						<tr style="background-color: #4747471a;">
							...
							<td data-title="Status" class="staus-dropdown dropdown">
								<button role="button" class="btn btn-waiting"<?php echo esc_attr( $backgroundg_color ); ?> data-toggle="dropdown">
									<?php echo esc_html( $order_status ); ?>
								</button>
								<ul class="dropdown-menu dropdown-menu-right">
									<li>
										<a href="#" data-postid="<?php echo get_the_ID(); ?>" data-orderid="<?php echo intval( $order_id ); ?>" class="quick-ship-wrh-pick_staged_ship" data-nonce="<?php echo esc_attr( wp_create_nonce( 'quick_ship_wrh_nonce' ) ); ?>">
											<?php esc_html_e( 'Staged To Ship', 'hoodslyhub' ); ?>
										</a>
									</li>
								</ul>
							</td>
			...
			<div class="hub-pagination" id="wrh_quick_shiping_list_paginate" data-max_num_pages="<?php echo $all_orders->max_num_pages; ?>">
				<?php
